...
* is partitioned by **day** (low-cardinality) and **SORTED BY ``person_id``**, so a
  per-user read prunes to a handful of files — without the partition-per-user
...
# The identity/event fields we lift out of otlp_logs into typed columns. This
# projection is valid both over the base table and over the change feed (both
# expose the same base columns). event_time is derived in UTC from the epoch-ns
# timestamp via microsecond precision; event_time_unix_nano keeps exact ordering.
_SELECT = """
...
_COLS = (
    "(event_time, event_time_unix_nano, event, user_id, anonymous_id, "
